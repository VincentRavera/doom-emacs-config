...
(defun edi-make-font-lock-keywords ()
  "Edifacts font configuration."
  `(
    (,(concat "ERC"
              "[" edi-data-element-separator "]"  "\\([A-Z0-9]+\\)")
     . font-lock-variable-name-face)
    (,(concat "CONTRL"
              edi-component-data-element-separator "\\([A-Z0-9]+\\)"
              edi-component-data-element-separator "\\([A-Z0-9]+\\)")
     . font-lock-variable-name-face)
    (,(concat "PAORE[SQ]"
              edi-component-data-element-separator "\\([A-Z0-9]+\\)"
    ...
    (,(concat "ILFSR[SQ]"
     ...
     . font-lock-keyword-face)
    ;; (,(concat "TVL"
    ;;           "[" edi-data-element-separator "]"  "\\([A-Z0-9" edi-component-data-element-separator "]+\\)")
    ;;  . font-lock-string-face)
    (,"TVL" . font-lock-string-face)
...
(defun edi-edi-to-readable ()
  "Make a 1 line EDI file into multiple lines.

By replacing the segment terminator ~ with a ~\n"
  (interactive)
  (save-excursion
	(goto-char (point-min))
